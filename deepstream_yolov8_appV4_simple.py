...
class SimpleDeepStreamPipeline:
    ...
    def on_new_sample(self, sink):
        ...
        if result:
            print(f"Buffer size: {map_info.size}, Width: {width}, Height: {height}")
            
            # Now we have direct access to the frame in BGRx format
            ...
            # Display the frame
            cv2.imshow("DeepStream Output", frame_bgr)
            cv2.waitKey(1)
